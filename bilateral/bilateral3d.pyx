...
def bilateral3d_optimized(np.ndarray[DTYPEfloat_t, ndim=3, mode="c"] data,voxel_size,double sigg,double sigi):
    ...
    g_kernel=np.asarray(gauss_kernel_3d(sigg, voxel_size),dtype=DTYPEfloat)
    #making it asimmetrical. The first element of new kernel should be the cenral element of old

    cdef np.ndarray[DTYPEfloat_t, ndim=3, mode="c"] gauss_kernel = g_kernel[ [slice(i/2,i) for i in g_kernel.shape] ]

    cdef unsigned int img_size_x, img_size_y, img_size_z = [for i in data.shape]
    cdef unsigned int ker_side_x, ker_side_y, ker_side_z = gauss_kernel.shape
                ...
                for x in range(img_size_x-ker_side_x):
                    for y in range(img_size_y-ker_side_y):
                        for z in range(img_size_z-ker_side_z):
                            ...
                            result_values [x, y, z] += img_value1*weight
                            result_values [x+ker_step_x, y+ker_step_y, z+ker_step_z] += img_value2*weight
